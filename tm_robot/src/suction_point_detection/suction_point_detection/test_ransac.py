...
class PlaneFittingNode(Node):
    def __init__(self):
        ...
        #recieve main controll cmd
        self.create_service(CaptureImage, 'grab_detect', self.fit_plane_callback)
        # 訂閱 ROI、點雲與彩色影像
    ...
    def fit_plane_callback(self,req,res):
        # 1) 根據 organized/unorganized 決定影像大小 H, W
        if self.latest_pc.height > 1:
            H, W = self.latest_pc.height, self.latest_pc.width
        else:
        ...
        region_pts = []
        for idx, p in enumerate(pc2.read_points(self.latest_pc, field_names=('x','y','z'), skip_nans=False)):
            row = idx // W
            if row < ymin:
                continue
            if row >= ymax:
        ...
        marker = Marker()
        marker.header = self.latest_pc.header
