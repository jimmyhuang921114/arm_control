...
class CameraPublisher(Node):
    ...
    def socket_handler(self):
        self.get_logger().info("SecondCamera called. Waiting for 4 signals on port 6001...")
        ...
        try:
            while count < 4:
                ...
                if (b'\x01' in data) and (now - self.last_capture_time) > 0.5:
                    ...
                    if ok:
                        count += 1
                        self.get_logger().info(f"拍攝中: {count}/4 -> {filename}")
                    else:
    ...
    def combine_image(self, tile_size=1024):
        names = ["img1.jpg", "img2.jpg", "img3.jpg", "img4.jpg"]
        imgs = []
        for n in names:
            p = os.path.join(self.capture_dir, n)
            if not os.path.exists(p):
                self.get_logger().warn(f"{p} file not exist")
                return
            img = cv2.imread(p)
            if img is None:
                self.get_logger().warn(f"can not read iamge: {p}")
                return
        ...
        if cv2.imwrite(out, combined):
            self.get_logger().info(f"image save at : {out}")
        else:
